...
export default function Library() {
  ...
  const [libraryNo, setLibraryNo] = useState("");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!libraryNo) {
      alert("Please enter the Library number/name.");
      return;
    }
    ...
    try {
      ...
      setFeedbackText("");
      setImageFile(null);
      setRatings({ seating: 0, books: 0, cleanliness: 0 });
        ...
        >
          ...
          <div className="w-full md:w-2/3 p-8 md:p-12">
            ...
            <p className="text-gray-600 mb-10 text-lg">
              Is the library quiet and well-stocked? Help us make it the perfect study spot.
            </p>

            <div className="space-y-8">
              {/* Title + Input */}
              <div className="flex flex-col gap-2">
                <label className="text-lg font-bold text-emerald-800">Library Section / No.</label>
                <input
                  type="text"
                  value={libraryNo}
                  onChange={(e) => setLibraryNo(e.target.value)}
                  placeholder="e.g. Main Library Floor 1"
              ...
              <div className="space-y-6 bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100">
                <RatingRow
                  label="Seating, Lighting, & Ambience"
                  value={ratings.seating}
                  onChange={(val) => handleRating("seating", val)}
                />
                <RatingRow
                  label="Availability & Quality of Books"
                  value={ratings.books}
                  onChange={(val) => handleRating("books", val)}
                />
                <RatingRow
              ...
              {/* Feedback Box */}
              <div className="flex flex-col gap-2">
                <label className="text-lg font-bold text-emerald-800">Your Suggestions</label>
                <textarea
                  placeholder="Share your experience or book requests..."
                  rows={4}
                  value={feedbackText}
