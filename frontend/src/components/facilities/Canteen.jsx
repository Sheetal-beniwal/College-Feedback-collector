...
export default function Canteen() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!canteenNo) {
      alert("Please enter the Canteen name/number.");
      return;
    }
        ...
        >
          ...
          <div className="w-full md:w-2/3 p-8 md:p-12">
            ...
            <p className="text-gray-600 mb-10 text-lg">
              From flavor to freshness — tell us how your canteen experience was!
            </p>

            <div className="space-y-8">
              {/* Title + Input */}
              <div className="flex flex-col gap-2">
                <label className="text-lg font-bold text-emerald-800">Canteen Name / No.</label>
                <input
                  type="text"
                  value={canteenNo}
              ...
              <div className="space-y-6 bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100">
                ...
                <RatingRow
                  ...
                  onChange={(val) => handleRating("foodQuality", val)}
                />
                <RatingRow
              ...
              {/* Feedback Box */}
              <div className="flex flex-col gap-2">
                <label className="text-lg font-bold text-emerald-800">Your Dish Reviews</label>
                <textarea
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pt-4">
                <label className="cursor-pointer border-2 border-dashed border-emerald-300 px-6 py-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-semibold transition-all w-full sm:w-auto text-center">
                  {imageFile ? "✅ Food Picture Selected" : "📷 Upload Food Picture"}
                  <input
                    type="file"
                    className="hidden"
